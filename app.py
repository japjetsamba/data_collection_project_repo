...
if css_path.exists():
    ...
    st.markdown('<style>div.stButton > button{background:#E65100;color:#fff;border:1px solid #E65100}</style>', unsafe_allow_html=True)

SCRAPE_URLS = {'Chiens':'https://sn.coinafrique.com/categorie/chiens','Moutons':'https://sn.coinafrique.com/categorie/moutons','Poules-Lapins-Pigeons':'https://sn.coinafrique.com/categorie/poules-lapins-et-pigeons','Autres animaux':'https://sn.coinafrique.com/categorie/autres-animaux'}
DEFAULT_PAGES = 2
...
menu = st.sidebar.selectbox('Choisir une page', ('Accueil','Scraper','Web Scraper (CSV brut)','Dashboard (nettoye)','Feedback'), index=0)
...
def show_scraper():
    ...
    st.caption('Choisissez la categorie et le nombre de pages, puis lancez le scraping. Le robot visite les pages detail et enregistre directement en SQLite.')
    c1, c2 = st.columns(2)
    with c1: category = st.selectbox('Categorie', list(SCRAPE_URLS.keys()), index=0)
    with c2: pages = st.slider('Pages', 1, 10, DEFAULT_PAGES)
    if st.button('Lancer le scraping et enregistrer en DB', type='primary'):
        with st.spinner('Scraping + insertion en base (Selenium Manager)…'):
            try:
                n = scraping.selenium_scrape_insert(category, 1, pages, headless=True, visit_detail=True)
                st.success(f'Termine — {n} lignes envoyees en DB (brut).')
            except Exception as e:
                st.error(f'Erreur : {e}')
    if st.button('Afficher les donnees en DB'):
        try:
            df_db = dbutils.fetch_all_raw()
            if df_db is None or df_db.empty: st.info('La base est vide.')
            else:
...
def show_ws_csv():
    st.header('WEB SCRAPER (CSV brut)')
    st.caption('Cliquez sur une categorie pour afficher les CSV bruts (extension Web Scraper).')
    ...
    if 'ws_choice_file' not in st.session_state: st.session_state.ws_choice_file = None
    ...
    if not st.session_state.ws_choice_file: st.info('En attente dune selection…'); return
    path = WS_DIR / st.session_state.ws_choice_file
    if not path.exists(): st.error(f'Fichier introuvable : {path.name}'); return
    try: df = pd.read_csv(path)
    except Exception as e: st.error(f'Lecture impossible : {e}'); return
    st.subheader(f'Apercu — {path.name}')
    st.write(f'Taille : {df.shape[0]} lignes x {df.shape[1]} colonnes')
    st.dataframe(df.head(100), use_container_width=True)

def show_dashboard():
    st.header('DASHBOARD (DONNEES NETTOYEES)')
    st.caption('Diagrammes construits a partir des CSV nettoyes (Web Scraper -> nettoyage).')
    ...
    if not frames: st.warning('Aucun CSV nettoye. Deposez dabord des bruts en Option Web Scraper.'); return
